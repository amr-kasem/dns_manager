...
class MethodChannelDnsManager extends DnsManagerPlatform {
  ...
  Future<String?> getDNS() async {
    ...
    _eventController.add(
      DnsOperationEvent(operation: 'getDNS', status: 'started'),
    );

    _executeOperation(
      'getDNS',
      () => methodChannel.invokeMethod<String>('getDNS'),
    );
    return null;
  }
  ...
  Future<String?> setDNS(String dns) async {
    // Return immediately and publish result via stream
    _eventController.add(
      DnsOperationEvent(operation: 'setDNS', status: 'started'),
    );

    _executeOperation(
      'setDNS',
      () => methodChannel.invokeMethod<String>('setDNS', {'dns': dns}),
    );
    return null;
  }
  ...
  Future<String?> resetDNS() async {
    // Return immediately and publish result via stream
    _eventController.add(
      DnsOperationEvent(operation: 'resetDNS', status: 'started'),
    );

    _executeOperation(
      'resetDNS',
      () => methodChannel.invokeMethod<String>('resetDNS'),
    );
    return null;
  }

  /// Execute operation asynchronously and publish results via stream
  static void _executeOperation(
    String operation,
    Future<String?> Function() methodCall,
  ) async {
    try {
    ...
    for (int i = 0; i < 15; i++) {
      ...
      try {
        ...
        if (status?.contains('connected') == true ||
            status?.contains('activated') == true) {
          _eventController.add(
            DnsOperationEvent(
              operation: 'connectionStatus',
              status: 'completed',
